...
const login = async (req, res) => {
    ...
    if (!username || !password) return res.redirect("/login?errors=All fields required");
    
    const user = await findUserByUsername(username);

    if (!user) return res.redirect("/login?errors=Invalid credentials");

    const isValid = await validatePassword(password, user.password);
    if (!isValid) return res.redirect("/login?errors=Invalid credentials")
